...
do
    echo "Starting $i"
    bash ./invoke_whitefield.sh ./config/wf-contiki-ngBOF.cfg && sleep 2700
    BRCOLOR=cadetblue1 DEFCOLOR=chocolate1 ./scripts/wfshell plot_network_graph t$i-BOF.png p$i-BOF.png scale=20
    for j in {1..16}
	do
		./scripts/wfshell cmd_mac_stats $(($j-1)) > statsofnodes/statsBOF-node$j-$i.txt
	done
...
do
    echo "Starting $i"
    bash ./invoke_whitefield.sh ./config/wf-contiki-ngN.cfg && sleep 2700
    BRCOLOR=cadetblue1 DEFCOLOR=chocolate1 ./scripts/wfshell plot_network_graph t$i-norm.png p$i-norm.png scale=20
    for j in {1..16}
        do
                ./scripts/wfshell cmd_mac_stats $(($j-1)) > statsofnodes/statsN-node$j-$i.txt
        done
    bash ./scripts/wfshell stop_whitefield
...
do
    echo "Starting $i"
    bash ./invoke_whitefield.sh ./config/wf-contiki-ngB.cfg && sleep 2700
    BRCOLOR=cadetblue1 DEFCOLOR=chocolate1 ./scripts/wfshell plot_network_graph t$i-B.png p$i-B.png scale=20
    for j in {1..16}
        do
                ./scripts/wfshell cmd_mac_stats $(($j-1)) > statsofnodes/statsB-node$j-$i.txt
        done
    bash ./scripts/wfshell stop_whitefield
    mv log log-ids-B-$i
    echo "Stopped $i"
